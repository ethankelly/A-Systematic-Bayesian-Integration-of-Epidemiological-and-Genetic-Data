...
/*
 key data structures (see header file for details): nt_struct (for holding sequence data); epi_struct (holding epi
 data); lh_SQUARE (holding likelihood which is sudivided into many blocks); para_key, key model parameters; para_aux,
 other unchanged model parameters e.g. type of spatial kernel
*/

/*
 Note: likelihood which is sudivided into many blocks so that we can perform partial updates of the likelihood to save
 some computational time; but this trick may work only for a general SEIR framework used in the paper. You may have to
 modify the code in other cases
*/

/*
 key classes (see header file for details): I defined a class mcmc_UPDATE so that I can use many variables, e.g. n_base
 (look for n_base_CUPDATE), as private and do not need to pass them in the function arguments; see header file,
 mcmc_UPDATE; you of course can pass those variables in function arguments and do not use the class mcmc_UPDATE
*/

/* Similarly for class Func */

/*
 key functions: t_e_seq, update infection time and seq jointly; source_t_e_update, update infection time, source of
 infection and sequence jointly (see the paper)
*/
...
//RInside R(argc, argv);
    ...
    IO_simdata(para_true, para_other, coordinate, epi_final, nt_data, index,
               con_seq_estm); //Importing epidemiological data, estimated master sequence (con_seq_estm) . etc
...
/*----------------------------*/
    ...
    for (int i = 0; i <= (int) (para_other.n - 1); i++) {
        if ((epi_final.infected_source.at(i) != 9999) & (epi_final.infected_source.at(i) != -99))
            xi_beta_E.push_back(i);
    }
...
//------------------------------------//
    ...
    vector<vector<double> > kernel_mat_current(para_other.n, vector<double>(para_other.n)), delta_mat_current(
            para_other.n, vector<double>(para_other.n)); // a dynamic matrix contain the "kernel distance"
    vector<double> norm_const_current(para_other.n);

//---------------------------------------------------//

    initialize_mcmc(para_current, para_other, xi_I_current, xi_U_current, xi_E_current, xi_E_minus_current,
                    xi_R_current, xi_EnI_current, xi_EnIS_current, xi_InR_current, t_e_current, t_i_current,
                    t_r_current, index_current, stb_current, gp_stb_current, infected_source_current,
                    kernel_mat_current, norm_const_current, sample_data, t_onset, nt_data_current,
                    con_seq_current); // initialze the parameters/unobserved data for mcmc
...
//////------------------------------------------------------------------------//////
    ...
    func_mcmc.set_para(para_current, para_other, coordinate, xi_U_current, xi_E_current, xi_E_minus_current,
                       xi_I_current, xi_R_current, xi_EnI_current, xi_InR_current, t_e_current, t_i_current,
                       t_r_current, stb_current, index_current, infected_source_current);
    ...
    func_mcmc.initialize_lh_square(lh_square_current, kernel_mat_current, delta_mat_current, norm_const_current,
                                   nt_data_current, con_seq_current); //initialize lh_square
...
//-------------------
    ...
    count_type_all(nt_data_current, xi_E_current, para_other.n_base, total_count_1_initial, total_count_2_initial,
                   total_count_3_initial);
...
//-------------
    ...
    myfile_out.close();
    myfile_out.open((string(path4) + string("initial_h_E.txt")).c_str(), ios::app);
    for (int i = 0; i <= ((int) xi_E_minus_current.size() - 1); i++) {
...
/*--------------------*/
    ...
    myfile1_out.open((string(path4) + string("parameters_current.txt")).c_str(),
                     ios::app); // for outputting current mcmc values of model parameters
    myfile1_out << "alpha" << "," << "beta" << "," << "mu_lat" << "," << "var_lat" << "," << "c" << "," << "d" << ","
                << "k_1" << "," << "k_2" << "," << "stb_1" << "," << "stb_2" << "," << "mu_1" << "," << "mu_2" << ","
                << "p_ber" << endl;
    myfile2_out.open((string(path4) + string("lh_current.txt")).c_str(), ios::app); // .. current likelihood value
    myfile4_out.open((string(path4) + string("infected_source_current.txt")).c_str(),
                     ios::app); // ..current imputed infected sources
    myfile10_out.open((string(path4) + string("con_seq_current.txt")).c_str(),
                      ios::app);// .. current imputed master seq
...
//gsl_rng_set (r_c, -1000*(i+1)); // set a seed
        ...
        mcmc_update.k_1_update(lh_square_current, log_lh_current, kernel_mat_current, delta_mat_current, xi_U_current,
                               xi_E_minus_current, xi_I_current, t_r_current, t_i_current, t_e_current, index_current,
                               para_current, stb_current, infected_source_current, norm_const_current, i,
                               r_c_unvs); // mcmc update for spatial kerenel parameter
...
//mcmc_update.k_2_update(lh_square_current, log_lh_current, kernel_mat_current, delta_mat_current, xi_U_current, xi_E_minus_current, xi_I_current, t_r_current,  t_i_current, t_e_current, index_current, para_current, stb_current, infected_source_current,  norm_const_current, i, r_c_unvs); 
        ...
        mcmc_update.beta_update(lh_square_current, log_lh_current, xi_U_current, xi_E_minus_current, t_e_current,
                                index_current, stb_current, infected_source_current, para_current, i,
                                r_c_unvs); // .. secondary transmisison rate

        mcmc_update.alpha_update(lh_square_current, log_lh_current, xi_U_current, xi_E_minus_current, t_e_current,
                                 index_current, stb_current, infected_source_current, para_current, i,
                                 r_c_unvs); // .. primary rate

        mcmc_update.mu_lat_update(lh_square_current, log_lh_current, xi_I_current, xi_EnI_current, t_i_current,
                                  t_e_current, index_current, para_current, i, r_c_unvs); // ...mean of latent period
        mcmc_update.var_lat_update(lh_square_current, log_lh_current, xi_I_current, xi_EnI_current, t_i_current,
                                   t_e_current, index_current, para_current, i,
                                   r_c_unvs); // .. variance of latent peiod

        mcmc_update.c_update(lh_square_current, log_lh_current, xi_R_current, xi_InR_current, t_r_current, t_i_current,
                             index_current, para_current, i, r_c_unvs); // .. parameter for infectious period
//mcmc_update.d_update(lh_square_current,log_lh_current, xi_R_current, xi_InR_current,t_r_current, t_i_current, index_current, para_current, i, r_c_unvs);
        ...
        mcmc_update.mu_1_update(lh_square_current, log_lh_current, xi_E_current, para_current, nt_data_current, i,
                                r_c_unvs); // .. mutation rate
        mcmc_update.mu_2_update(lh_square_current, log_lh_current, xi_E_current, para_current, nt_data_current, i,
                                r_c_unvs);

        mcmc_update.p_ber_update(lh_square_current, log_lh_current, xi_E_current, para_current, nt_data_current,
                                 infected_source_current, con_seq_current, i,
                                 r_c_unvs); //.. variation parameter for the Master sequence G_M

        for (int k = 0; k <= (para_other.n_base - 1); k++) {
            mcmc_update.con_seq_update(lh_square_current, log_lh_current, kernel_mat_current, delta_mat_current,
                                       xi_U_current, xi_E_current, xi_E_minus_current, xi_I_current, xi_EnI_current,
                                       t_r_current, t_i_current, t_e_current, index_current, para_current, stb_current,
                                       norm_const_current, infected_source_current, nt_data_current.t_sample,
                                       nt_data_current.current_size, nt_data_current.nt, nt_data_current.t_nt,
                                       xi_beta_E_current, con_seq_current, k, r_c_unvs); // update the master sequence
        }
...
//--------------------//
        ...
        for (int j = 0; j <= ((int) (n_freq / 10.0) - 1); j++) { // update sequence without changing t_e, source
            ...
            int subject_proposed_2 = xi_E_current.at(gsl_rng_uniform_int(r_c_unvs,
                                                                         xi_E_current.size())); // gsl_rng_uniform_int : a int random number in [0,xi_E_arg.size()-1] will be drawn

            for (int k = 0; k <= (para_other.n_base - 1); k++) {
                mcmc_update.seq_update(lh_square_current, log_lh_current, kernel_mat_current, delta_mat_current,
                                       xi_U_current, xi_E_current, xi_E_minus_current, xi_I_current, xi_EnI_current,
                                       t_r_current, t_i_current, t_e_current, index_current, para_current, stb_current,
                                       norm_const_current, infected_source_current, nt_data_current.t_sample,
                                       nt_data_current.current_size, nt_data_current.nt, nt_data_current.t_nt,
                                       xi_beta_E_current, con_seq_current, subject_proposed_2, k, r_c_unvs);
            }
...
//--------------------//
        ...
        for (int j = 0; j <= (n_freq - 1); j++) {
            ...
            int subject_proposed = xi_E_current.at(gsl_rng_uniform_int(r_c_unvs,
                                                                       xi_E_current.size())); // gsl_rng_uniform_int : a int random number in [0,xi_E_arg.size()-1] will be drawn

            mcmc_update.t_e_seq(lh_square_current, log_lh_current, kernel_mat_current, delta_mat_current, xi_U_current,
                                xi_E_current, xi_E_minus_current, xi_I_current, xi_EnI_current, t_r_current,
                                t_i_current, t_e_current, index_current, para_current, stb_current, norm_const_current,
                                infected_source_current, nt_data_current.t_sample, nt_data_current.current_size,
                                nt_data_current.nt, nt_data_current.t_nt, nt_data_current.infecting_list,
                                nt_data_current.infecting_size, xi_beta_E_current, con_seq_current, subject_proposed,
                                i + 1, r_c_unvs); // update t_e and sequence jointly

            mcmc_update.source_t_e_update(lh_square_current, log_lh_current, kernel_mat_current, delta_mat_current,
                                          xi_U_current, xi_E_current, xi_E_minus_current, xi_I_current, xi_EnI_current,
                                          t_r_current, t_i_current, t_e_current, index_current, para_current,
                                          stb_current, norm_const_current, infected_source_current,
                                          nt_data_current.t_sample, nt_data_current.current_size, nt_data_current.nt,
                                          nt_data_current.t_nt, nt_data_current.infecting_list,
                                          nt_data_current.infecting_size, xi_beta_E_current, subject_proposed,
                                          list_update, con_seq_current, i + 1,
                                          r_c_unvs); // update t_e, source, sequence, jointly
        ...
        for (int jk = 0; jk <= ((int) index_current.size() - 1); jk++) {// update the  first seq of indexes
            for (int k = 0; k <= (para_other.n_base - 1); k++) {
                mcmc_update.seq_update(lh_square_current, log_lh_current, kernel_mat_current, delta_mat_current,
                                       ...
                                       norm_const_current, infected_source_current, nt_data_current.t_sample,
                                       nt_data_current.current_size, nt_data_current.nt, nt_data_current.t_nt,
                                       xi_beta_E_current, con_seq_current, index_current.at(jk), k, r_c_unvs);
            }

        }


        mcmc_update.t_i_update(lh_square_current, log_lh_current, kernel_mat_current, delta_mat_current, xi_U_current,
                               xi_E_current, xi_E_minus_current, xi_I_current, xi_EnI_current, xi_R_current,
                               xi_InR_current, t_r_current, t_i_current, t_onset, t_e_current, index_current,
                               para_current, stb_current, norm_const_current, infected_source_current,
                               nt_data_current.t_sample, nt_data_current.current_size, nt_data_current.nt,
                               nt_data_current.t_nt, nt_data_current.infecting_list, nt_data_current.infecting_size,
                               i + 1);


//--------------

        myfile1_out << para_current.alpha << "," << para_current.beta << "," << para_current.mu_lat << ","
                    << para_current.var_lat << "," << para_current.c << "," << para_current.d << "," << para_current.k_1
                    << "," << para_current.k_2 << "," << para_current.stb_1 << "," << para_current.stb_2 << ","
                    << para_current.mu_1 << "," << para_current.mu_2 << "," << para_current.p_ber << endl;
//myfile1_out.close();
        ...
        if (div_iter_lh.rem ==
            0) { // for outputting infected_source (only at every 100 iterations, which saves space..)
...
// for outputting the master sequence (only at every 2000 iterations, which saves space..)
        if (div_iter_seq.rem == 0) {
